...
public class Bicycle extends Transport {
    ...
    public boolean drive(int distance, TerrainTypes terrainType) {
        if (Arrays.asList(restrictedTerrainTypes).contains(terrainType)) {
            System.out.println("На велосипеде нельзя перемещаться по типу местности: " + terrainType);
            return false;
        }
        System.out.println("Перемещение на ТС: " + name + " по типу местности: " + terrainType + " на расстояние: " + distance);
        return true;
    }
}
